...
class PerceptualLoss(nn.Module):
    def __init__(self, loss=perceptual_calc_loss, blocks=[0, 2, 4], weights=[0.2, 0.3, 0.5], device=None):
        super().__init__()
...
def perceptual_loss(blocks=[0, 2, 4], weights=[0.2, 0.3, 0.5], device=None):
    if not device:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    return PerceptualLoss(perceptual_calc_loss, blocks, weights, device)
